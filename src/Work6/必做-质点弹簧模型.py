...
def init_cloth():
    """Python 侧按序调用各初始化 kernel，保证 GPU 状态同步"""
    num_springs[None] = 0  
    init_positions()
    init_springs()
    init_spring_indices()
...
def compute_forces_on(pos, vel, force):
    ...
    for s in range(num_springs[None]):
        ...
        if dist > 1e-6:
            d_norm = d / dist
            f_spring = -k_s * (dist - spring_lengths[s]) * d_norm
            ti.atomic_add(force[idx_a],  f_spring)
            ti.atomic_add(force[idx_b], -f_spring)

@ti.func
def clamp_velocity(vel, idx: int):
...
def main():
    ...
    window = ti.ui.Window("Mass-Spring Cloth Simulation", (900, 900))
    ...
    while window.running:
        ...
        window.GUI.begin("Control Panel", 0.02, 0.02, 0.40, 0.42)
        ...
        window.GUI.text(f"Grid: {N}x{N}  Springs: {num_springs[None]}")
